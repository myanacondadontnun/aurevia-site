...
function SuccessModal({ email, onClose }: { email: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className={`relative w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#0a0a0a] shadow-[0_20px_80px_rgba(2,223,166,0.08)] overflow-hidden transition-all duration-500 ${showContent ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-4"}`}
      >
        {/* Green glow bar */}
        <div className="h-1 w-full bg-gradient-to-r from-transparent via-[#02DFA6] to-transparent" />

        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-white/30 hover:text-white/70 hover:bg-white/5 transition-all duration-200"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="px-8 pt-10 pb-8">
          ...
          <div className="relative w-16 h-16 mx-auto mb-6">
            <div className={`absolute inset-0 rounded-full bg-[#02DFA6]/10 transition-transform duration-700 ease-out ${showContent ? "scale-100" : "scale-0"}`} />
            <div className={`absolute inset-2 rounded-full bg-[#02DFA6]/20 transition-transform duration-700 delay-100 ease-out ${showContent ? "scale-100" : "scale-0"}`} />
            <div className={`absolute inset-0 flex items-center justify-center transition-all duration-500 delay-300 ${showContent ? "opacity-100 scale-100" : "opacity-0 scale-50"}`}>
              <Check className="w-8 h-8 text-[#02DFA6]" strokeWidth={2.5} />
            </div>
          </div>

          {/* Headline */}
          <h3 className="text-2xl font-inter font-normal text-white text-center mb-2">
            You&apos;re all set
          </h3>
  ...
  return (
    <>
      ...
      <div id="free-audit" className="scroll-mt-28">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-0">
              ...
              <div className="relative flex-1 group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Mail className="w-4 h-4 text-white/30 group-focus-within:text-[#02DFA6] transition-colors duration-200" aria-hidden="true" />
                </div>
                <input
                  type="email"
                  name="email"
                  ...
                  className="w-full pl-11 pr-3 py-3 sm:py-3.5 bg-transparent text-sm text-white placeholder-white/30 focus:outline-none rounded-xl"
                />
              </div>
              ...
              <div className="relative flex-1 group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Globe className="w-4 h-4 text-white/30 group-focus-within:text-[#02DFA6] transition-colors duration-200" aria-hidden="true" />
                </div>
                <input
                  type="text"
                  name="storeUrl"
        ...
        <p className="text-[11px] text-white/25 mt-3 ml-1">
