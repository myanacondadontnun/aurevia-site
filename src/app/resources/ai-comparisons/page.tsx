...
const articleLink =
  "font-medium text-sky-400 underline decoration-sky-400/40 underline-offset-[3px] hover:text-[#02DFA6] hover:decoration-[#02DFA6]";
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-white/5">
        <div
          className="h-full rounded-full bg-gradient-to-r from-[#02DFA6]/70 to-[#02DFA6]"
          style={{ width: `${value}%` }}
        />
      </div>
    </div>
  ...
  return (
    <Card
      className={`overflow-hidden border transition-all duration-300 ${
        agent.winner
          ? "border-[#02DFA6]/50 bg-gradient-to-br from-[#02DFA6]/10 via-card/80 to-card shadow-[0_0_40px_-12px_rgba(2,223,166,0.35)]"
          : "border-border/50 bg-card/30 hover:border-[#02DFA6]/25"
      }`}
    >
          ...
          <div className="flex items-start gap-3">
            <span
              className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-sm font-medium ${
                agent.winner
                  ? "bg-[#02DFA6] text-black"
                  : "border border-border/60 bg-white/5 text-muted-foreground"
              }`}
            >
              {agent.rank}
            </span>
            <div>
              <h3 className="text-lg font-normal text-white sm:text-xl">{agent.name}</h3>
              <p className="mt-1 max-w-prose text-sm font-light leading-relaxed text-muted-foreground">
                {agent.philosophy}
              </p>
            </div>
          </div>
          <span className="rounded-full border border-[#02DFA6]/25 bg-[#02DFA6]/10 px-3 py-1 text-xs font-medium text-[#02DFA6]">
            {agent.price}
          </span>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <span className="rounded-md border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-white/90">
            <span className="text-[#02DFA6]">+ </span>
            {agent.strength}
          </span>
          <span className="rounded-md border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-muted-foreground">
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto rounded-xl border border-border/50 bg-card/20 shadow-inner">
        <table className="min-w-[720px] w-full border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-[#02DFA6]/20 bg-[#02DFA6]/[0.07]">
              {headers.map((h, i) => (
                <th
                  key={h}
                  className={`px-4 py-3 font-medium text-white/95 ${
          ...
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr
                  key={row[0]}
                  className={`border-b border-border/30 transition-colors hover:bg-white/[0.03] ${
                    isAurevia ? "bg-[#02DFA6]/[0.06]" : ""
                  }`}
                >
                  {row.map((cell, i) => (
                    <td
                      ...
                      className={`px-4 py-3 font-light text-muted-foreground ${
                        i === 0
                          ? `sticky left-0 z-10 font-normal text-white backdrop-blur-sm ${
        ...
        <thead>
          <tr className="border-b border-border/50">
            {QUICK_COMPARISON.headers.map((h) => (
              <th key={h} className="px-4 py-3.5 text-sm font-semibold text-white">
                {h}
              </th>
            ))}
          </tr>
            ...
            return (
              <tr
                ...
                className={`border-b border-border/30 last:border-0 ${
                  isAurevia ? "bg-[#02DFA6]/[0.06]" : "hover:bg-white/[0.02]"
                }`}
              >
                {row.map((cell, i) => (
                  <td
...
function FeatureCell({ value }: { value: string | boolean }) {
  if (value === true) {
    return (
      <span className="inline-flex items-center justify-center rounded-full bg-[#02DFA6]/15 p-1 text-[#02DFA6]">
  ...
  if (value === false) {
    return (
      <span className="inline-flex items-center justify-center rounded-full bg-white/5 p-1 text-muted-foreground">
...
function FeatureMatrixTable({ rows }: { rows: FeatureMatrixRow[] }) {
  return (
    <div className="overflow-x-auto rounded-xl border border-[#02DFA6]/20 bg-card/25">
      <table className="min-w-[800px] w-full border-collapse text-left text-sm">
        <thead>
          <tr className="border-b border-border/40">
            <th className="sticky left-0 z-20 bg-background/98 px-4 py-4 font-medium text-white backdrop-blur-sm">
              Feature / capability
            </th>
            <th className="border-l-2 border-[#02DFA6] bg-[#02DFA6]/[0.08] px-4 py-4 font-medium text-[#02DFA6]">
              Aurevia.io
            </th>
            <th className="px-4 py-4 font-medium text-white/90">Zipchat AI</th>
            <th className="px-4 py-4 font-medium text-white/90">Tidio (Lyro)</th>
            <th className="px-4 py-4 font-medium text-white/90">Gorgias</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.feature} className="border-b border-border/25 hover:bg-white/[0.02]">
              <td className="sticky left-0 z-10 bg-background/95 px-4 py-3.5 font-normal text-white backdrop-blur-sm">
                {row.feature}
              </td>
              <td className="border-l-2 border-[#02DFA6]/60 bg-[#02DFA6]/[0.04] px-4 py-3.5">
  ...
  return (
    <PageLayout>
      <div className="container mx-auto max-w-[1320px] px-4 sm:px-6">
        <Link
          href="/resources/"
          className="mb-6 inline-flex items-center gap-1 text-sm text-muted-foreground transition-colors hover:text-[#02DFA6]"
        >
        ...
        <AIComparisonListicleLayout>
          <Fragment>
            ...
            <header className="mb-10 sm:mb-12">
              <div className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5 text-white/90">
                  <span className="font-medium text-white">Aurevia Research</span>
                  <span className="text-muted-foreground/60">·</span>
                  Written for merchants
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <CheckCircle2 className="h-3.5 w-3.5 text-[#02DFA6]" aria-hidden />
                  Expert verified
                </span>
                <span>Last updated April 2026</span>
              </div>
              <h1 className="max-w-4xl text-3xl font-normal leading-tight text-white sm:text-4xl md:text-[2.65rem]">
              ...
              <p className="mt-5 max-w-3xl text-base font-light leading-relaxed text-muted-foreground sm:text-lg">
                ...
                <strong className="font-medium text-white/90">paid traffic that does not convert</strong>, you need a
              ...
              <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                  ...
                  <div
                    key={s.label}
                    className="rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-center sm:text-left"
                  >
                    <p className="text-xl font-normal text-white sm:text-2xl">{s.value}</p>
            ...
            <InfographicCapabilities />
            <h3 className="mt-12 text-lg font-normal text-white sm:text-xl">Three architectures you will see</h3>
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
              <Card className="border-border/50 bg-card/40 transition-all hover:border-[#02DFA6]/30">
                <CardContent className="p-5">
                  <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-[#02DFA6]/10 text-[#02DFA6]">
                    <Headphones className="h-5 w-5" />
                  </div>
                  <h4 className="text-base font-normal text-white">Support-centric</h4>
              ...
              <Card className="border-border/50 bg-card/40 transition-all hover:border-[#02DFA6]/30">
                <CardContent className="p-5">
                  ...
                  <h4 className="text-base font-normal text-white">Chat hybrids</h4>
                  <p className="mt-2 text-sm font-light leading-relaxed text-muted-foreground">
              ...
              <Card className="border-border/50 bg-card/40 transition-all hover:border-[#02DFA6]/30">
                <CardContent className="p-5">
                  ...
                  <h4 className="text-base font-normal text-white">Proactive sales agents</h4>
                  <p className="mt-2 text-sm font-light leading-relaxed text-muted-foreground">
          ...
          <section id="why-you-need" className="scroll-mt-28">
            ...
            <ul className="mt-6 list-disc space-y-3 pl-5 text-base font-light leading-relaxed text-muted-foreground">
              <li>
                <strong className="font-medium text-white">Always-on revenue defense:</strong> shoppers on mobile,
                international time zones, and promo spikes still get guided answers.
              </li>
              <li>
                <strong className="font-medium text-white">Higher conversion without more headcount:</strong>{" "}
              ...
              <li>
                <strong className="font-medium text-white">Cleaner ops:</strong> move repetitive product Q&amp;A out
          ...
          <section id="how-we-compared" className="scroll-mt-28">
            <h2 className="text-2xl font-normal text-white sm:text-3xl">How we compared these tools</h2>
            <p className="mt-4 text-base font-light leading-relaxed text-muted-foreground">
          ...
          <section id="full-landscape" className="scroll-mt-28">
            <h2 className="text-2xl font-normal text-white sm:text-3xl">The full vendor landscape (all tiers)</h2>
            ...
            <div className="mt-10 space-y-16">
              {TIER_SECTIONS.map((tier) => (
                <div key={tier.id}>
                  <div className="mb-5">
                    <span className="text-xs font-medium uppercase tracking-widest text-[#02DFA6]">
                      {tier.tierLabel}
                    </span>
                    <h3 className="mt-1 text-xl font-normal text-white sm:text-2xl">{tier.title}</h3>
          ...
          <section id="deep-tables" className="scroll-mt-28 space-y-14">
            <h2 className="text-2xl font-normal text-white sm:text-3xl">Category deep-dives (tabular)</h2>
          ...
          <section id="feature-matrix" className="scroll-mt-28">
            <h2 className="text-2xl font-normal text-white sm:text-3xl">Feature matrix: Aurevia vs alternatives</h2>
          ...
          <section
            id="verdict"
            className="scroll-mt-28 rounded-2xl border border-[#02DFA6]/25 bg-gradient-to-br from-[#02DFA6]/10 via-transparent to-transparent p-6 sm:p-10"
            aria-labelledby="verdict-heading"
          >
            <h2 id="verdict-heading" className="text-2xl font-normal text-white sm:text-3xl">
              Verdict: why Aurevia wins the revenue use case
            </h2>
            <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
              <Card className="border-border/40 bg-black/25">
                <CardContent className="p-5">
                  <Zap className="mb-3 h-8 w-8 text-[#02DFA6]" />
                  <h3 className="text-lg font-normal text-white">Guided checkout flow</h3>
              ...
              <Card className="border-border/40 bg-black/25">
                <CardContent className="p-5">
                  <TrendingUp className="mb-3 h-8 w-8 text-[#02DFA6]" />
                  <h3 className="text-lg font-normal text-white">Upsell architecture</h3>
                  <p className="mt-2 text-sm font-light text-muted-foreground">
              ...
              <Card className="border-border/40 bg-black/25">
                <CardContent className="p-5">
                  <LineChart className="mb-3 h-8 w-8 text-[#02DFA6]" />
                  <h3 className="text-lg font-normal text-white">Disruptive economics</h3>
                  <p className="mt-2 text-sm font-light text-muted-foreground">
            ...
            <blockquote className="mt-8 border-l-2 border-[#02DFA6] pl-5 text-base font-light italic leading-relaxed text-muted-foreground sm:text-lg">
          ...
          <div className="mt-12 rounded-xl border border-[#02DFA6]/20 bg-gradient-to-br from-[#02DFA6]/5 to-transparent p-6 sm:p-8">
            <h3 className="text-lg font-normal text-white sm:text-xl">Ready to ship your AI sales co-pilot?</h3>
            ...
            <div className="flex flex-wrap gap-3">
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full bg-[#02DFA6] px-5 py-2.5 text-sm font-medium text-black transition-colors duration-200 hover:bg-[#02DFA6]/90"
              >
                Try Aurevia Free
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-full border border-border/50 px-5 py-2.5 text-sm font-medium text-muted-foreground transition-all duration-200 hover:border-[#02DFA6]/30 hover:text-white"
              >
                <ArrowLeft className="h-4 w-4" />
                More guides
              </Link>
